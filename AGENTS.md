...
トップディレクトリの [`mise.toml`](./mise.toml) にタスクを定義している。`mise tasks` で一覧表示、`mise run <task>` で実行。

`.env` から `REMOTE_URL` / `FLY_APP_NAME` / `FLY_ACCESS_TOKEN` を自動読み込み。`APP_URL` のデフォルトは `http://localhost:3000`。
...
| ディレクトリ / ファイル | 内容 |
|--------|------|
| `_local/har/` | HAR ファイル (初期計測・比較用) |
| `_local/screenshots/` | スクリーンショット |
