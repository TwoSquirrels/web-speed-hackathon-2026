...
export const ImageArea = ({ fetchpriority, images, loading = "lazy" }: Props) => {
  return (
      ...
      <div className="border-cax-border grid h-full w-full grid-cols-2 grid-rows-2 gap-1 overflow-hidden rounded-lg border">
        {images.map((image, idx) => {
          return (
            ...
            >
              <CoveredImage alt={image.alt} fetchpriority={fetchpriority} loading={loading} src={getImagePath(image.id)} />
            </div>
          );
        })}
      </div>
